...
func (l gormzap) Trace(
	...
	case l.SlowQueryThreshold != logger.DisableThreshold && elapsedTime > l.SlowQueryThreshold && l.LogLevel >= gormLogger.Warn:
		sql, rowsAffected := fc()
		l.logger().Warn(
			fmt.Sprintf("query_slow (>= %vms)", l.SlowQueryThreshold),
...
func (l gormzap) TraceTransaction(ctx context.Context, begin time.Time) {
	if l.LogLevel <= gormLogger.Silent {
		return
	}
	...
	case l.SlowTransactionThreshold != logger.DisableThreshold && elapsed > l.SlowTransactionThreshold && l.LogLevel >= gormLogger.Warn:
		l.logger().Warn(
			fmt.Sprintf("transaction_slow (>= %vms)", l.SlowQueryThreshold),
...
func (l gormzap) ParamsFilter(ctx context.Context, sql string, params ...interface{}) (string, []interface{}) {
