...
/* This function exchanges the OIDC code to get the OIDC tokens given by the provider, then a new authenticated user is created
in the backend storage. The session_code corresponding to the new user is sent back to the frontend */
func GetSessionCode(w http.ResponseWriter, request *http.Request) {
	...
	sessionCode := NewSessionCode(email, tokens)
	if error != "" {
...
/* This function uses the refresh_token to call the RefreshTokens method of the Provider struct.
The session of the user is refreshed and the new session_code is sent to the frontend */
...
/* This function only sends a json {"status":"authenticated"} as it is only reachable if the session of the user is valid
The checking of the session_code is made in the MiddlewareAuthenticator */
