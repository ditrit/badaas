...
func (p GoogleProvider) GetTokens(code string) (models.Tokens, string, string, error) {
	...
	if err != nil {
		return models.Tokens{}, "", "", fmt.Errorf("Failed to exchange tokens")
	}

	accessToken, ok := oauth2Token.Extra("access_token").(string)
	log.Println("accessToken: " + accessToken + "\n")
	if !ok {
		return models.Tokens{}, "", "", fmt.Errorf("Failed to extract the access_token")
	}
	rawIDToken, ok := oauth2Token.Extra("id_token").(string)
	log.Println("rawIDToken: " + rawIDToken + "\n")
	if !ok {
		return models.Tokens{}, "", "", fmt.Errorf("Failed to extract the id_token")
	}
	...
	if err != nil {
		return models.Tokens{}, "", "", fmt.Errorf("Failed to verify id_token")
	}
	var IDTokenClaims *json.RawMessage = new(json.RawMessage)
	idToken.Claims(&IDTokenClaims)

	IDTokenBody := map[string]string{}
	err = json.Unmarshal(*IDTokenClaims, &IDTokenBody)
	if err != nil {
		return models.Tokens{}, "", "", fmt.Errorf("Failed to extract data from IDToken")
	}
