...
func (sessionService *sessionServiceImpl) add(session *models.Session) error {
	...
	err := sessionService.sessionRepository.Create(sessionService.db.GormDB, session)
	if err != nil {
		return err
	}
...
func (sessionService *sessionServiceImpl) pullFromDB() {
	...
	sessionsFromDatabase, err := sessionService.sessionRepository.Query(sessionService.db.GormDB)
	if err != nil {
		panic(err)
	}
...
func (sessionService *sessionServiceImpl) removeExpired() {
	...
	for sessionUUID, session := range sessionService.cache {
		if session.IsExpired() {
			...
			err := sessionService.sessionRepository.Delete(sessionService.db.GormDB, session)
			if err != nil {
				panic(err)
			}
...
func (sessionService *sessionServiceImpl) RollSession(sessionUUID badorm.UUID) httperrors.HTTPError {
	...
	if session.CanBeRolled(rollInterval) {
		...
		err := sessionService.sessionRepository.Save(sessionService.db.GormDB, session)
		if err != nil {
