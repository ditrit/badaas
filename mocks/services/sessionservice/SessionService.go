...
func (_m *SessionService) IsValid(sessionUUID badorm.UUID) (bool, *sessionservice.SessionClaims) {
	ret := _m.Called(sessionUUID)

	var r0 bool
	var r1 *sessionservice.SessionClaims
	if rf, ok := ret.Get(0).(func(badorm.UUID) (bool, *sessionservice.SessionClaims)); ok {
		return rf(sessionUUID)
	}
	if rf, ok := ret.Get(0).(func(badorm.UUID) bool); ok {
		r0 = rf(sessionUUID)
	} else {
		r0 = ret.Get(0).(bool)
	}

	if rf, ok := ret.Get(1).(func(badorm.UUID) *sessionservice.SessionClaims); ok {
		r1 = rf(sessionUUID)
	} else {
		if ret.Get(1) != nil {
...
func (_m *SessionService) RollSession(_a0 badorm.UUID) httperrors.HTTPError {
	ret := _m.Called(_a0)

	var r0 httperrors.HTTPError
	if rf, ok := ret.Get(0).(func(badorm.UUID) httperrors.HTTPError); ok {
		r0 = rf(_a0)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(httperrors.HTTPError)
